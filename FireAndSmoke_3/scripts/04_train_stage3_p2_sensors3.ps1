...
$Model = Join-Path $Project "models\yolov8m-p2-fire-smoke-3cls.yaml"
$Runs = Join-Path $Root "runs"

if (-not (Test-Path $Data)) { throw "Missing dataset yaml: $Data. Run scripts\02_build_strict_datasets.ps1 first." }
if (-not $Pretrained) {
  $candidate = Join-Path $Root "initial_weights\sensors_yolov8m_3cls_100_best.pt"
  if (Test-Path $candidate) { $Pretrained = $candidate } else { $Pretrained = "yolov8m.pt" }
}

Write-Host "== Train YOLOv8m-P2 Sensors 3-class =="
