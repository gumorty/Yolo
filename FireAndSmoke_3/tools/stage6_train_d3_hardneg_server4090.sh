...
WORKERS="${WORKERS:-12}"
EPOCHS="${EPOCHS:-8}"
HARD_REPEAT="${HARD_REPEAT:-8}"

PROJECT="${PROJECT:-$ROOT/runs_stage6_server4090}"
RUN_NAME="${RUN_NAME:-stage6_d3_hardneg_calibration_x${HARD_REPEAT}_i${IMGSZ}}"
GENERATED_DIR="$ROOT/generated"
GENERATED_DATA="$GENERATED_DIR/stage6_d3_hardneg_x${HARD_REPEAT}_server.yaml"
mkdir -p "$GENERATED_DIR"

SOURCE_WEIGHT="${SOURCE_WEIGHT:-$PROJECT/stage6_d2_server4090_phase2_unfreeze_s5_union_i960/weights/best.pt}"
if [[ ! -f "$SOURCE_WEIGHT" ]]; then
  echo "Missing D2 source weight: $SOURCE_WEIGHT" >&2
  echo "Set SOURCE_WEIGHT=/path/to/best.pt if needed." >&2
  exit 1
fi
...
echo "D3 hard-negative calibration complete."
