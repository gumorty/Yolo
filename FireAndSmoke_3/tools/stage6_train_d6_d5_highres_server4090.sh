...
RUN_NAME="${RUN_NAME:-stage6_d6_d5_highres_x${HARD_REPEAT}_i${IMGSZ}}"
GENERATED_DIR="$ROOT/generated"
GENERATED_DATA="$GENERATED_DIR/stage6_d6_d5_highres_x${HARD_REPEAT}_server.yaml"
mkdir -p "$GENERATED_DIR"

SOURCE_WEIGHT="${SOURCE_WEIGHT:-$PROJECT/stage6_d5_s5_hardneg_balanced_x4_i960/weights/best.pt}"
if [[ ! -f "$SOURCE_WEIGHT" ]]; then
  echo "Missing D5 source weight: $SOURCE_WEIGHT" >&2
  echo "Set SOURCE_WEIGHT=/path/to/d5/best.pt if needed." >&2
  exit 1
fi
...
python tools/train_yolo_api.py \
  ...
  --weight-decay 0.0005 \
  --warmup-epochs 0.0 \
  --warmup-bias-lr 0.00002 \
  --freeze 10 \
  --amp true \
  --save-period 1

echo "D6 high-resolution fine-tuning complete."
